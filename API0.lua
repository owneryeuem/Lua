...
function Analytics.new(config)
	assert(type(config.url) == "string", "OWNERTRUEV2Analytics: config.url is required")
	assert(type(config.token) == "string", "OWNERTRUEV2Analytics: config.token is required")
...
local KICK_WATCHER_MAX_POLLS = 1800 
...
function Analytics:windowCreated(data)
	...
	local ok2, result2 = pcall(function()
		return _getgenv().OWNERTRUEV2_ASSET_ID
	end)
	if ok2 and type(result2) == "number" then
		customAssetId = true
	end
	local ok3, result3 = pcall(function()
		return _getgenv().OWNERTRUEV2_SECURE
	end)
